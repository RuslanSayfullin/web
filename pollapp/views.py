...
def survey_create(request):
    ...
    if serializer.is_valid():
        ...
        return Response(PollSerializer(survey).data, status=status.HTTP_201_CREATED)
...
def survey_update(request, survey_id):
    survey = get_object_or_404(Poll, pk=survey_id)
    if request.method == 'PATCH':
        serializer = PollSerializer(survey, data=request.data, partial=True)
        if serializer.is_valid():
            survey = serializer.save()

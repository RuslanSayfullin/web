...
class ApiCaller:
    ...
    def __calc_signature(self, params):
        ...
        return hashlib.md5(str(params_str + self.secret_key).encode('utf-8')).hexdigest()
    ...
    def __prepare_api_params(self, format, method_name, params, session_key, uid):
        ...
        for param in api_params:
            api_params[param] = api_params[param]
    ...
    def execute(self, method_name, params, session_key_or_uid, format=FORMAT_JSON, method=METHOD_GET):
        ...
        if method == METHOD_GET:
            req = BASE_URL.encode('utf-8') + urllib.parse.urlencode(api_params).encode('utf-8')
        else:
            req = urllib2.Request(BASE_URL, urllib.parse.urlencode(api_params))
        # try:
...
class PyMyMailRu:
    ...
    def achievements_add(self, img_url, description, width, group_id, session_key):
        return self.execute('achievements.add',
                            {'img_url': img_url, 'description': description, 'width': width, 'group_id': group_id},
    ...
    def achievements_group_update(self, group_id, description, session_key):
        return self.execute('achievements.group_update', {'group_id': group_id, 'description': description},
                            session_key, self.format)
    ...
    def achievements_update(self, achievement_id, img_url, description, weight, session_key):
        return self.execute('achievements.update',
                            {'id': achievement_id, 'img_url': img_url, 'description': description, 'weight': weight},
                            session_key, self.format)
    ...
    def audio_search(self, query, offset, limit, session_key_or_uid):
        return self.execute('audio.search', {'query': query, 'offset': offset, 'limit': limit}, session_key_or_uid,
                            self.format)
    ...
    def guestbook_post(self, uid, title, description, user_text, img_url, img_file, link1_text, link1_href, link2_text,
                       link2_href, session_key):
        return self.execute('guestbook.post',
                            {'uid': uid, 'title': title, 'description': description, 'user_text': user_text,
    ...
    def messages_get_thread(self, uid, offset, limit, session_key):
        return self.execute('messages.getThread', {'uid': uid, 'offset': offset, 'limit': limit}, session_key,
                            self.format)

    def messages_get_threads_list(self, uid, offset, limit, session_key):
        return self.execute('messages.getThreadsList', {'uid': uid, 'offset': offset, 'limit': limit}, session_key,
                            self.format)
    ...
    def multipost_send(self, uid2, text, photo, audio, video, session_key):
        return self.execute('multipost.send',
                            {'uid2': uid2, 'text': text, 'photo': photo, 'audio': audio, 'video': video}, session_key,
                            self.format)
    ...
    def stream_get(self, offset, limit, filter_app, session_key_or_id):
        return self.execute('stream.get', {'offset': offset, 'limit': limit, 'filrer_app': filter_app},
                            session_key_or_id,
                            self.format)

    def stream_get_by_author(self, uid, offset, limit, filter_app, session_key):
        return self.execute('stream.getByAuthor',
                            {'uid': uid, 'offset': offset, 'limit': limit, 'filter_app': filter_app}
                            , session_key, self.format)
    ...
    def stream_post(self, user_text, text, title, link1_text, link1_href, link2_text, link2_href, img_url,
                    session_key_or_id):
        return self.execute('stream.post',
                            {'user_text': user_text, 'text': text, 'title': title, 'link1_text': link1_text,
                             'link1_href': link1_href, 'link2_text': link2_text, 'link2_href': link2_href,

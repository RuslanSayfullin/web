...
class CreateUpdateDogovorEntry(generic.FormView):
    template_name = 'dogovora/create_update_entry.html'
    form_class = forms.DogovorFormEntry
    froze = None
    froze_uuid = None
    dogovor = None
    create_or_update = None
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.create_or_update == 'update' and (self.dogovor.nomer_dogovora is None or self.dogovor.nomer_dogovora == ''):
            try:
                dogovora_s_takimi_zhe_fio = DogovorEntry.objects.filter(
                    passport_familiya=self.dogovor.passport_familiya,
                    passport_imya=self.dogovor.passport_imya,
                    passport_otchestvo=self.dogovor.passport_otchestvo) \
                    .exclude(passport_familiya='', passport_imya='', passport_otchestvo='') \
                    .exclude(tip_dogovora__in=('entity_textile',
                                               'juridical_moscow_ooo-refabrik_textile',
                                               'entity_ufa_textile',
                                               'entity_ufa_ooo-refabrik_textile')) \
                    .exclude(id=self.dogovor.id).exclude(nomer_dogovora=None).exclude(nomer_dogovora='').count()
                drugie_dogovora = DogovorEntry.objects.filter(
                    passport_familiya=self.dogovor.passport_familiya,
                    passport_imya=self.dogovor.passport_imya,
                    passport_otchestvo=self.dogovor.passport_otchestvo) \
                    .exclude(passport_familiya='', passport_imya='', passport_otchestvo='') \
    ...
    def dispatch(self, *args, **kwargs):
        ...
        self.froze = get_object_or_404(Froze, uuid=self.froze_uuid)
        try:
            self.dogovor = DogovorEntry.objects.get(froze_id=self.froze)
            self.create_or_update = 'update'
        except ObjectDoesNotExist:
            self.create_or_update = 'create'
        return super(CreateUpdateDogovorEntry, self).dispatch(*args, **kwargs)
    ...
    def get_initial(self):
        if self.create_or_update == 'update':
            ...
            initial = {
                ...
                'letter_of_attorney': self.dogovor.letter_of_attorney if self.dogovor.letter_of_attorney else None,
                'tax_identification_number': self.dogovor.tax_identification_number if self.dogovor.tax_identification_number else None,
                'registration_reason_code': self.dogovor.registration_reason_code if self.dogovor.registration_reason_code else None,
                'registration_number': self.dogovor.registration_number if self.dogovor.registration_number else None,
                'checking_account': self.dogovor.checking_account if self.dogovor.checking_account else None,
                'account_bank': self.dogovor.account_bank if self.dogovor.account_bank else None,
                'correspondent_account': self.dogovor.correspondent_account if self.dogovor.correspondent_account else None,
                'bank_identification_code': self.dogovor.bank_identification_code if self.dogovor.bank_identification_code else None,
                'adres_ustanovki': self.dogovor.adres_ustanovki if self.dogovor.adres_ustanovki else None,
                'vsego_k_oplate': self.dogovor.vsego_k_oplate if self.dogovor.vsego_k_oplate or self.dogovor.vsego_k_oplate == 0 else None,
                'oplata_predoplata_rub': self.dogovor.oplata_predoplata_rub if self.dogovor.oplata_predoplata_rub or self.dogovor.oplata_predoplata_rub == 0 else None,
                'naimenov_soputstv_izdeliy': self.dogovor.naimenov_soputstv_izdeliy if self.dogovor.naimenov_soputstv_izdeliy else None,
            ...
            return super(CreateUpdateDogovorEntry, self).get_initial()
    ...
    def form_valid(self, form):
        ...
        letter_of_attorney = form.cleaned_data.get('letter_of_attorney') if form.cleaned_data.get('letter_of_attorney') else None
        tax_identification_number = form.cleaned_data.get('tax_identification_number') if form.cleaned_data.get('tax_identification_number') else None
        registration_reason_code = form.cleaned_data.get('registration_reason_code') if form.cleaned_data.get('registration_reason_code') else None
        registration_number = form.cleaned_data.get('registration_number') if form.cleaned_data.get('registration_number') else None
        checking_account = form.cleaned_data.get('checking_account') if form.cleaned_data.get('checking_account') else None
        account_bank = form.cleaned_data.get('account_bank') if form.cleaned_data.get('account_bank') else None
        ...
        if self.create_or_update == 'update':
            ...
            self.dogovor.letter_of_attorney = letter_of_attorney if letter_of_attorney else None
            self.dogovor.tax_identification_number = tax_identification_number if tax_identification_number else None
            self.dogovor.registration_reason_code = registration_reason_code if registration_reason_code else None
            self.dogovor.registration_number = registration_number if registration_number else None
            self.dogovor.checking_account = checking_account if checking_account else None
            self.dogovor.account_bank = account_bank if account_bank else None
            self.dogovor.correspondent_account = correspondent_account if correspondent_account else None
            self.dogovor.bank_identification_code = bank_identification_code if bank_identification_code else None
            self.dogovor.adres_ustanovki = adres_ustanovki if adres_ustanovki else None
            self.dogovor.vsego_k_oplate = vsego_k_oplate if vsego_k_oplate or vsego_k_oplate == 0 else None
            self.dogovor.oplata_predoplata_rub = oplata_predoplata_rub if oplata_predoplata_rub or oplata_predoplata_rub == 0 else None
            self.dogovor.naimenov_soputstv_izdeliy = naimenov_soputstv_izdeliy if naimenov_soputstv_izdeliy else None

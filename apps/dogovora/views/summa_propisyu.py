...
def num_words_forms(num, the_forms):
    form_for_1, form_for_2, form_for_5 = the_forms
    num = int(num)
    if len(str(num)) >= 2:
        num = int(str(num)[-2:])
        if 20 > num > 10:  # если число принадлежит отрезку [11;19]
            return form_for_5
        num = int(str(num)[-1:])
    if num == 1:  # если оканчивается на 1
        return form_for_1
    elif 5 > num > 1:  # если число оканчивается на 2, 3, 4
        return form_for_2
    return form_for_5
